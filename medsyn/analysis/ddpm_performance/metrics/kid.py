...
"""
KID (Kernel Inception Distance) Metric Computation Script

This script computes KID metrics comparing synthetic images against ground truth:
- Per-class KID with mean and standard deviation
- Global KID across all classes
...
3. Computing KID for each synthetic subset vs. full ground truth
4. Taking mean and std across subset KID values
...
def compute_kid_per_class(
    ...
    """
    Compute KID mean and std for a single class.
    ...
    Args:
        ...
        class_id: Class to compute KID for
        counts_per_computation: Number of synthetic samples per subset
        device: Device for computation ('cuda' or 'cpu')
        weights_path: Path to Inception weights (optional)
        subset_size: Size of subsets for KID computation
        kid_subsets: Number of subsets in KID algorithm
    ...
    try:
        ...
        logger.error(
            "torchmetrics KID not available. "
    ...
    # Compute KID for each subset
    kid_values = []
    for subset_idx, synth_subset in enumerate(synth_subsets):
        try:
            # Create KID computer
            kid_computer = KernelInceptionDistance(
                feature=2048,
            ...
            kid_computer = kid_computer.to(device)

            # Add real images (all of them)
            with torch.no_grad():
                real_batch = real_images_class.to(device)
                kid_computer.update(real_batch, real=True)
                del real_batch
                if device == "cuda":
            ...
            with torch.no_grad():
                ...
                kid_computer.update(synth_batch, real=False)
                del synth_batch
                if device == "cuda":
                    torch.cuda.empty_cache()

            # Compute KID (returns mean and std, we take the mean)
            kid_result = kid_computer.compute()
            kid_value = float(kid_result[0].cpu().item())  # mean
            ...
            logger.error(f"Class {class_id}, subset {subset_idx}: KID computation failed: {e}")
            continue

    if len(kid_values) == 0:
        logger.warning(f"Class {class_id}: No valid KID values computed")
        return float("nan"), float("nan")

    # Compute mean and std
    mean_kid = float(np.mean(kid_values))
    std_kid = float(np.std(kid_values, ddof=1) if len(kid_values) > 1 else 0.0)

    logger.info(
        f"Class {class_id}: KID = {mean_kid:.4f} +- {std_kid:.4f} "
        f"(n={len(kid_values)} subsets)"
    )

    return mean_kid, std_kid


def compute_kid_global(
    ...
    """
    Compute global KID mean and std across all classes.
        ...
        try:
            ...
            with torch.no_grad():
                ...
                for i in range(0, len(real_images), chunk_size):
                    ...
                    kid_computer.update(real_chunk, real=True)
                    del real_chunk
                    if device == "cuda":
                        torch.cuda.empty_cache()
            ...
            logger.debug(
                f"Global subset {subset_idx + 1}/{num_subsets}: KID = {kid_value:.4f}"
            )
            ...
            logger.error(f"Global subset {subset_idx}: KID computation failed: {e}")
            continue

    if len(kid_values) == 0:
        logger.warning("Global: No valid KID values computed")
        return float("nan"), float("nan")
    ...
    logger.info(
        f"Global: KID = {mean_kid:.4f} +- {std_kid:.4f} "
        f"(n={len(kid_values)} subsets)"
    )
...
def compute_kid_for_model(
    ...
    """
    Compute KID metrics with std for a single model.

    This is the main computation function that:
    1. Preprocesses images to torchmetrics format
    2. Computes per-class KID with mean/std
    3. Computes global KID with mean/std
    ...
    logger.info("Computing per-class KID metrics...")
    class_means = []
    class_stds = []

    for class_id in tqdm(range(num_classes), desc="Computing per-class KID"):
        mean_kid, std_kid = compute_kid_per_class(
            real_images,
            real_labels,
            synth_images,
            synth_labels,
        ...
        results[f"class_{class_id}_mean"] = mean_kid
        results[f"class_{class_id}_std"] = std_kid

        if not np.isnan(mean_kid):
            class_means.append(mean_kid)
            class_stds.append(std_kid)
    ...
    # Compute global KID (optional - commented out by default to save compute)
    # Uncomment if you want global KID in addition to per-class
    # logger.info("Computing global KID metric...")
    # global_mean, global_std = compute_kid_global(
    #     real_images,
    #     real_labels,
    #     synth_images,
    #     synth_labels,
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Compute KID metrics comparing synthetic images to ground truth",
...
Example usage:
  python kid.py \\
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Size of subsets for KID computation (default: 100)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of subsets to use in KID (default: 100)",
    )
    ...
    if args.subset_size < 1:
        parser.error("--subset-size must be at least 1")

    if args.kid_subsets < 1:
        parser.error("--kid-subsets must be at least 1")

    return args


def main():
    """Main entry point for KID computation script."""
    args = parse_args()

    # Setup logging
    setup_logging(args.verbose)
    logger.info("Starting KID computation")
    logger.info(f"Device: {args.device}")
    logger.info(f"Counts per computation: {args.counts}")
    logger.info(f"Number of classes: {args.num_classes}")
    logger.info(f"KID subset size: {args.subset_size}")
    logger.info(f"KID subsets: {args.kid_subsets}")
    ...
    for model_name, model_path_str in args.model:
        ...
        # Compute KID metrics
        logger.info(f"Computing KID metrics for {model_name}...")
        model_results = compute_kid_for_model(
            gt_images,
            gt_labels,
            synth_images,
            synth_labels,
        ...
        logger.info(f"  Average KID: {model_results['average_mean']:.4f} +- {model_results['average_std']:.4f}")
    ...
    if all_results:
        ...
        logger.info("KID computation completed successfully!")
        return 0
    else:
