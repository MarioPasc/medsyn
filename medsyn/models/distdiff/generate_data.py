...
def main(args):
    ...
    # (Fixed for parallel generation across multiple GPUs)
    # # 获取当前时间
    # now = datetime.now()
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        for image_i in range(args.first_image_index, args.num_images_per_prompt):
            ...
            with (accelerator.accumulate([unet])):
                ...
                # Use is_local_main_process for logging to avoid excessive logs while allowing all processes to log
                if accelerator.is_local_main_process: logging.info("Guidance timesteps: %s", ", ".join([str(x) for x in guide_timesteps]))
                for i, t in tqdm_bar(enumerate(timesteps[start_index:]),
                    ...
                    if t == guide_timesteps[0] and args.guidance_type == "transform_guidance":
                        ...
                        if accelerator.is_local_main_process: logging.info(
